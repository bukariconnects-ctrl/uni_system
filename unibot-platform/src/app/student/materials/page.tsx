...
export default async function StudentMaterialsPage() {
  ...
  // ── Course plans (syllabi) ────────────────────────────
  let syllabi: any[] = [];
  if (courseIds.length > 0) {
    const { data } = await serviceClient
      .from("syllabi")
      .select("*, courses(code, name), profiles!syllabi_instructor_id_fkey(first_name, last_name)")
      .in("course_id", courseIds)
  ...
  return (
    <div>
      <div className="mb-6">
        ...
        <p className="mt-1 text-sm text-text-secondary">تصفح وتحميل المواد التعليمية وخطط المقررات</p>
      </div>
