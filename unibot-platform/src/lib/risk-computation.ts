 ...
 * Extracted so it can be called directly from both the API route
 * and server actions — avoiding an internal HTTP round-trip that
 * would be intercepted by middleware and redirected to login.
 */
export async function computeRiskScores(
  ...
  // Get enrollments by tenant + semester directly
  ...
  if (!enrollments || enrollments.length === 0) {
    return { computed: 0, course_flags: 0 };
  }

  const studentCoursePairs = enrollments.map((e: any) => ({
  ...
  const studentIds = [
    ...new Set(studentCoursePairs.map((p: any) => p.student_id)),
  ];
  const courseIds = [
    ...new Set(studentCoursePairs.map((p: any) => p.course_id)),
  ];

  // For attendance: use section_id from enrollment to join with attendance_summaries
  ...
  for (const pair of studentCoursePairs) {
    ...
    if (sectionIdForAtt) {
      const absencePct =
        summaryMap.get(`${pair.student_id}-${sectionIdForAtt}`) || 0;
    ...
    const studentSubs =
      submissionsByStudent.get(pair.student_id) || new Set();
  ...
  courseRiskMap.forEach((data, courseId) => {
    ...
    const flagged = failureRate >= 30 || data.highCount / data.total >= 0.3;
  ...
  // Also sync risk_level to student_profiles for dashboard queries
  for (const rs of riskScores) {
    await adminClient
      .from("student_profiles")
      .update({
        risk_level: rs.risk_level as any,
        risk_score: rs.risk_score,
        risk_updated_at: new Date().toISOString(),
      })
      .eq("profile_id", rs.student_id)
