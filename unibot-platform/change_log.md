...
## 2026-07-06 — Feature: Smart Recommendation Engine (نظام التوصيات الذكي)

### نظرة عامة
بناء محرك توصيات ذكي متكامل يعمل كمرشد أكاديمي شامل منفصل عن RAG Chatbot، مع 4 أنواع من التوصيات المدعومة بالذكاء الاصطناعي (Gemini 2.5 Flash).

### قاعدة البيانات و RLS
- إضافة عمود `rec_type VARCHAR(50)` مع CHECK constraint لقيم: `remedial`, `enrichment`, `on_demand`, `daily_tip`, `manual`
- تفعيل RLS مع 6 سياسات: الطلاب (SELECT فقط)، المحاضر (SELECT + INSERT)، الإدارة الأكاديمية (SELECT)
- إنشاء `v_academic_risk_students` view لحساب الطلاب المعرضين للخطر من بيانات الغياب

### 4 قوالب Prompt مركزية
- **remedialPrompt** — للطلاب ذوي الدرجات الضعيفة (<60%) مع خطة مراجعة وقنوات يوتيوب وكتب ومنصات تدريب
- **enrichmentPrompt** — للطلاب المتفوقين (>90%) مع مواضيع متقدمة وشهادات مهنية
- **onDemandPrompt** — ردود تفاعلية على أسئلة الطلاب مع شرح ومصادر تعليمية
- **dailyTipPrompt** — نصيحة يومية مخصصة بناءً على مقررات الطالب وتكاليفه القادمة وتعاميم المنصة
- **قواعد صارمة لمكافحة الـ Hallucination:** ممنوع كتابة روابط URLs مباشرة، فقط أسماء قنوات وكتب محددة

### تحسينات API
- `POST /api/ai/recommendations/auto` — توليد توصيات علاجية/إثرائية آلياً عند تسجيل الدرجات
- `POST /api/ai/recommendations/request` — توصيات تفاعلية حسب طلب الطالب
- `POST /api/ai/recommendations/daily` — نصيحة يومية مع سياق كامل (المقررات، التكاليف، التعاميم)
- `src/lib/ai/recommendation-engine.ts` — محرك التوصيات الخلفي المركزي

### واجهة المستخدم (Student UI)
- **بحث تفاعلي** "اطلب توصية تعليمية..." مع إدخال اختياري لاسم المادة
- **بطاقات ملونة** حسب `rec_type`:
  - 🔶 Remedial (برتقالي — AlertTriangle)
  - 🟢 Enrichment (أخضر — Star)
  - 🟣 On-Demand (بنفسجي — Lightbulb)
  - 🔵 Daily Tip (أزرق — Sparkles)
- تكامل مع `react-markdown` و `remark-gfm` لعرض المحتوى المنسق

### إصلاح مشكلة منطقة الخطر (Faculty Risk Zone)
- إعادة كتابة `getFacultyRiskScores()` لاستخدام `v_academic_risk_students` view بدلاً من جدول `student_risk_scores` الفارغ
- إضافة حقول `student_major_name` و `student_level_name` في عرض المحاضر وواجهة المستخدم

### الملفات
| الملف | التغيير |
|-------|---------|
| supabase/migrations/20260705000006_add_rec_type_rls_recommendations.sql | إنشاء — إضافة rec_type + RLS |
| src/lib/ai/recommendation-prompts.ts | إنشاء — 4 قوالب Prompt مركزية |
| src/app/api/ai/recommendations/daily/route.ts | إنشاء — API النصيحة اليومية |
| src/app/api/ai/recommendations/auto/route.ts | تعديل — استخدام القوالب المركزية |
| src/app/api/ai/recommendations/request/route.ts | تعديل — استخدام القوالب المركزية |
| src/lib/ai/recommendation-engine.ts | تعديل — استخدام القوالب المركزية |
| src/app/student/recommendations/page.tsx | تعديل — دعم daily_tip مع سياق كامل |
| src/app/student/recommendations/recommendations-client.tsx | تعديل — بطاقات ملونة حسب rec_type |
| src/app/faculty/reports/reports-client.tsx | تعديل — إضافة تخصص ومستوى الطالب |
| src/app/faculty/risk-zone/actions.ts | إعادة كتابة — استخدام v_academic_risk_students |
| src/app/faculty/risk-zone/risk-zone-client.tsx | تعديل — عرض تخصص ومستوى الطالب |

### النتيجة
- محرك توصيات ذكي متكامل بأربعة أنماط
- قوالب Prompt احترافية مع مكافحة Hallucination
- واجهة مستخدم بديهية مع بطاقات ملونة ومحتوى Markdown منسق
- إصلاح شامل لصفحة منطقة الخطر للمحاضر
- إضافة تخصص ومستوى الطالب في تقارير المحاضر
- البناء يمر بنجاح

---

## 2026-07-06 — Security Fix: إزالة بيانات الطلاب والمقررات من تقارير مدير المنصة

### المشكلة
صفحة تقارير مدير المنصة (Super Admin) كانت تعرض بيانات تعليمية تفصيلية لا تخصه:
- عدد الطلاب وأعضاء هيئة التدريس والمقررات عبر ALL tenants
- الأقسام والتخصصات وجلسات الحضور
- هذه البيانات تمثل **اختراقاً لعزل البيانات (Data Isolation)** و**مخاطرة أمنية**

### الإجراء
1. **حذف `v_super_admin_platform_stats` view** — كان يجمع إحصائيات من جداول `profiles`, `courses`, `departments`, `majors`, `enrollments`, `attendance_sessions` عبر كل المستأجرين دون عزل
2. **إزالة استعلام `platformStats`** من صفحة السيرفر `page.tsx`
3. **إعادة كتابة `reports-client.tsx`**:
   - 🗑️ إزالة KPIs: عدد الطلاب، أعضاء هيئة التدريس، إجمالي المقررات
   - 🗑️ إزالة grid "إحصائيات المنصة" (الأقسام، التخصصات، جلسات الحضور)
   - 🗑️ إزالة KPI "متوسط حد الغياب"
   - ✅ الإبقاء على: إجمالي الجامعات، التخزين، تكلفة AI، نمو الجامعات، استهلاك AI Tokens، حالة الجامعات، قائمة الجامعات

### الملفات
| الملف | التغيير |
|-------|---------|
| supabase/migrations/20260706000001_drop_super_admin_platform_stats.sql | إنشاء — حذف view يخترق عزل البيانات |
| src/app/super-admin/reports/page.tsx | تعديل — إزالة استعلام platformStats |
| src/app/super-admin/reports/reports-client.tsx | إعادة كتابة — عرض بيانات المنصة فقط |

### النتيجة
- مدير المنصة يرى فقط بيانات إدارة المنصة (الجامعات، التخزين، الاشتراكات)
- لا يمكنه الاطلاع على بيانات الطلاب والمقررات الداخلية للجامعات
- عزل بيانات كامل وآمن
- البناء يمر بنجاح

---

## 2026-07-06 — تحسين جودة التوصيات وإعادة تصميم صفحة الطالب

### تحسين جودة Prompts التوصيات
- **رفع maxTokens**: daily_tip (512 → 2048)، remedial/enrichment/on_demand (2048 → 4096)
- **رفع temperature**: daily_tip (0.6 → 0.7)، الباقي (0.4/0.5 → 0.5) لمحتوى أكثر طبيعية
- **إعادة كتابة prompts** لتكون أكثر تحديداً وفائدة:
  - **نصيحة اليوم**: طلب فقرتين منظمتين (نصيحة رئيسية + اقتراح إضافي) مع تنويع بين تقنيات الدراسة، إدارة الوقت، الصحة النفسية
  - **العلاجية**: طلب خطة مراجعة بخطوات محددة مع أوقات مقترحة وأهداف، أسماء فيديوهات محددة وليس فقط قنوات
  - **الإثرائية**: طلب شهادات مهنية محددة (CCNA, AWS, PMP)، ومسار تطوير واضح
  - **التفاعلية**: طلب استخدام تشبيهات من الحياة اليومية وأمثلة تطبيقية

### تحسين سياق النصيحة اليومية
- إضافة جلب `student_majors` → `majors.name` للحصول على **التخصص**
- إضافة جلب `enrollments` → `academic_levels.name` للحصول على **المستوى الأكاديمي**
- تمرير هذه المعلومات الإضافية إلى `dailyTipPrompt()` لكتابة نصائح أكثر تخصيصاً

### إعادة تصميم صفحة التوصيات (Student UI)
- **بطاقة "نصيحة اليوم"** منفصلة في الأعلى بتصميم hero مع خلفية متدرجة وتأثيرات decorative
- **شريط تبويبات** للتصفية:
  - 🔵 الكل — يعرض كل التوصيات ما عدا النصيحة اليومية
  - ✨ نصيحة اليوم — النصائح اليومية
  - ⭐ التوصيات الآلية — remedial + enrichment من AI
  - 👤 توصيات المحاضرين — التوصيات اليدوية من المحاضرين
  - 💬 طلباتي — on_demand الذي طلبها الطالب
- **بطاقات مطورة**: أيقونة بارزة بخلفية بيضاء وظل، عرض اسم المحاضر المرسل (مع join على profiles)، عداد لكل تبويب، زر "عرض المزيد" للمحتوى الطويل مع تأثير fade
- **حالات فارغة** لكل تبويب مع رسائل مخصصة وزر إجراء مناسب
- **زر طلب توصية** عائم (موبايل) + زر ظاهر (ديسكتوب) مع فتح/إغلاق النموذج
- **تحسين عرض التواريخ**: نص نسبي (منذ ساعة، أمس، منذ 3 أيام) مع عرض التاريخ الكامل عند التمرير

### الملفات
| الملف | التغيير |
|-------|---------|
| src/lib/ai/recommendation-prompts.ts | إعادة كتابة — prompts أعمق وأكثر تحديداً مع maxTokens أعلى |
| src/app/student/recommendations/page.tsx | تعديل — إضافة تخصص ومستوى الطالب لسياق النصيحة اليومية |
| src/app/student/recommendations/actions.ts | تعديل — إضافة join مع profiles لجلب اسم المحاضر |
| src/app/student/recommendations/recommendations-client.tsx | إعادة كتابة كاملة — تصميم حديث بتبويبات وبطاقات مطورة |

### النتيجة
- توصيات أعمق وأكثر فائدة بفضل الـ prompts المحسنة
- سياق أغنى للنصيحة اليومية (تخصص + مستوى أكاديمي)
- واجهة مستخدم منظمة وسهلة الاستخدام مع تبويبات وتصنيفات
