-- ============================================================
-- Fix remaining enrollment grade sync issues
-- ============================================================
-- PROBLEM:
--   The migration 20260707000006 only updated enrollments where
--   gradebook_entries.is_published = TRUE.  Enrollments whose
--   grades were unpublished still carry the old (wrong) total
--   and letter_grade.
--
--   This script:
--     A) Re-syncs ALL enrollments that have a matching
--        gradebook_entry, regardless of publish status.
--     B) Recalculates GPA for every affected student.
-- ============================================================

-- ── A. Temporarily disable GPA trigger ──
ALTER TABLE enrollments DISABLE TRIGGER trg_enrollments_update_gpa;

-- ── B. Sync final_grade + letter_grade for ALL enrollments ──
...
-- ── C. Re-enable the trigger ──
ALTER TABLE enrollments ENABLE TRIGGER trg_enrollments_update_gpa;

-- ── D. Recalculate GPA for ALL students that have ANY gradebook entry ──
