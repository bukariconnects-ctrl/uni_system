-- Fix: Don't dismiss students based on attendance records for active sessions
-- The issue was that when attendance records are created with 'absent' status,
-- the trigger immediately calculates 100% absence and dismisses the student.
-- 
-- Solution: Only count absences from CLOSED sessions (not active ones)
...
BEGIN
    ...
    -- Only count attendance from CLOSED sessions (not active/open ones)
    SELECT
        COUNT(*),
    ...
    WHERE ar.section_id = COALESCE(NEW.section_id, OLD.section_id)
      ...
      AND asess.is_open = FALSE;  -- Only count closed sessions
    ...
    -- Only dismiss if threshold exceeded AND we have at least 3 closed sessions
...
-- Also fix any enrollments that were incorrectly dismissed
UPDATE enrollments 
SET status = 'enrolled' 
WHERE status = 'dismissed' 
  AND id IN (
    SELECT e.id 
    FROM enrollments e
    LEFT JOIN attendance_summaries ats ON ats.enrollment_id = e.id
    WHERE e.status = 'dismissed'
      AND (ats.total_sessions IS NULL OR ats.total_sessions < 3)
  );
