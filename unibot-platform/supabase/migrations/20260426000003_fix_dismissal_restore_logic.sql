-- Bugfix: dismissal restore logic used stale v_enrollment_status snapshot.
-- Now uses v_is_dismissed (from attendance_summaries) which reflects the actual
-- persisted dismissed state at the time the trigger runs.
...
BEGIN
    ...
    -- Read persisted dismissed state from attendance_summaries
    ...
    -- Restore: use v_is_dismissed (persisted flag) NOT stale enrollment status snapshot
