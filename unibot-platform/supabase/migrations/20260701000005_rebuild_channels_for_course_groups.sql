-- Migration 18: Rebuild channels for course groups (course_id + major_id + academic_level_id + semester_id)
-- Sections no longer exist; channels must be keyed by the actual grouping dimensions.
-- ============================================================

-- Step 1: Add group-columns to channels
ALTER TABLE channels ADD COLUMN IF NOT EXISTS major_id         UUID REFERENCES majors(id) ON DELETE SET NULL;
ALTER TABLE channels ADD COLUMN IF NOT EXISTS academic_level_id UUID REFERENCES academic_levels(id) ON DELETE SET NULL;
ALTER TABLE channels ADD COLUMN IF NOT EXISTS semester_id      UUID REFERENCES semesters(id) ON DELETE SET NULL;

CREATE INDEX IF NOT EXISTS idx_channels_major_id          ON channels(major_id);
CREATE INDEX IF NOT EXISTS idx_channels_academic_level_id ON channels(academic_level_id);
CREATE INDEX IF NOT EXISTS idx_channels_semester_id       ON channels(semester_id);

-- Step 2: Wipe old course channels (CASCADE takes care of members + messages)
DELETE FROM channels WHERE channel_type = 'course';

-- Step 3: Unique index so we never create duplicate channels for the same group
...
-- Step 4: Helper function — find-or-create a channel for a course group
...
BEGIN
    ...
    -- Add all instructors who teach this course-group as admins
...
-- Step 5: Trigger function — auto-add/remove students on enrollment changes
...
-- Step 6: Backfill — create channels for every existing enrolled group
DO $$
DECLARE
    rec RECORD;
    ch_id UUID;
BEGIN
    ...
    RAISE NOTICE 'Migration 18 complete: channels rebuilt for course groups';
END $$;
